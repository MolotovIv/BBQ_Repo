...
const activeChats = new Map();        // clientId → Set(adminId) — используется для быстрого доступа
...
async function finalizeOrder(ctx) {
    ...
    // Сохраняем клиента в активные чаты (если ещё нет)
    if (!activeChats.has(userId)) {
        activeChats.set(userId, new Set(ADMIN_IDS));
    }
...
// ==================== ОБРАБОТЧИКИ СООБЩЕНИЙ (пересылка любых сообщений от клиентов) ====================
...
bot.on('text', async (ctx) => {
    ...
    // === Если это клиент (не админ) — пересылаем админам ===
    if (!ADMIN_IDS.includes(chatId)) {
        // Добавляем в активные чаты (если ещё нет)
        if (!activeChats.has(userId)) {
            activeChats.set(userId, new Set(ADMIN_IDS));
            console.log(`➕ Клиент ${userId} добавлен в activeChats.`);
        }
        ...
        for (const adminId of ADMIN_IDS) {
            try {
                ...
                console.error(`Ошибка пересылки админу ${adminId}:`, error);
            }
        }
        await ctx.reply('✅ Ваше сообщение отправлено менеджеру. Мы ответим в ближайшее время.');
        return; // не обрабатываем как команду или комментарий
    }
    ...
    // === Если админ отвечает на уведомление (Reply) ===
    if (message.reply_to_message) {
        ...
        if (orderNotifications.has(repliedMsgId)) {
            ...
            try {
                ...
                orderNotifications.delete(repliedMsgId);
                await ctx.reply('✅ Ответ отправлен клиенту.');
            } catch (error) {
                console.error('Ошибка отправки ответа клиенту:', error);
                await ctx.reply('❌ Не удалось отправить ответ клиенту.');
            }
            return;
        }
    }

    // === Логика для клиентов (комментарии к заказу) — только если не админ и не команда ===
...
// 2. Обработчик для медиа-сообщений (фото, видео, голосовые и т.д.)
bot.on('message', async (ctx) => {
    // Если это текстовое сообщение, пропускаем (уже обработано выше)
    if (ctx.message.text) return;

    const chatId = ctx.chat.id;
    const userId = ctx.from.id;

    // Пересылаем только от клиентов (не админов)
    if (!ADMIN_IDS.includes(chatId)) {
        // Добавляем в активные чаты (если ещё нет)
        if (!activeChats.has(userId)) {
            activeChats.set(userId, new Set(ADMIN_IDS));
            console.log(`➕ Клиент ${userId} добавлен в activeChats (медиа).`);
        }
        // Пересылаем медиа-сообщение админам
        for (const adminId of ADMIN_IDS) {
            try {
                await ctx.forwardMessage(adminId);
            } catch (error) {
                console.error(`Ошибка пересылки медиа админу ${adminId}:`, error);
            }
        }
        await ctx.reply('✅ Ваше сообщение (медиа) отправлено менеджеру.');
    }
});
